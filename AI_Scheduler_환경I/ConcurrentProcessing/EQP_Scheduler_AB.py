...
class Wafer:
    def __init__(self, wafer_id, wafer_type: WaferType):
        self.id = wafer_id
...
class Wafer:
    def __init__(self, wafer_id, wafer_type: WaferType):
        ...
        # Define process times for each wafer type
        if wafer_type == WaferType.TYPE_A:
            self.process_time = 30
        elif wafer_type == WaferType.TYPE_B:
            self.process_time = 90
        elif wafer_type == WaferType.TYPE_C:
            self.process_time = 100
        elif wafer_type == WaferType.TYPE_D:
            self.process_time = 50
...
class EQP_Scheduler:
    ...
    def reset(self, seed=None):
        ...
        for i in range(self.max_wafer_no):
            # wafer_type = np.random.choice(list(WaferType), p=[0.67, 0.33, 0.0, 0.0, 0.0])                 # 2 Wafers
            # wafer_type = np.random.choice(list(WaferType), p=[0.5, 0.3, 0.2, 0.0, 0.0])                   # 3 Wafers
            wafer_type = np.random.choice(list(WaferType), p=[0.4, 0.3, 0.2, 0.1, 0.0])                     # 4 Wafers
            self.wafers.append(Wafer(i, wafer_type))
    ...
    def getCMBExeTime(self, wafer: Wafer):
        if wafer is None:
            raise ValueError("No wafer provided to getCMBExeTime.")
        
        chamber_speed_factors_A = [1, 1, 1, 1, 1, 1]
        chamber_speed_factors_B = [1.3, 1.3, 1.15, 1.15, 1, 1]
        chamber_speed_factors_C = [1.5, 1.5, 1.25, 1.25, 1, 1]
        chamber_speed_factors_D = [1.5, 1.5, 1.25, 1.25, 1, 1]

        current_pending_components = [
            c for c in self.components[ComponentType.CHAMBER]
            if c.pending_action and 'CHAMBER_' in c.pending_action.name
        ]

        if current_pending_components:
            chamber = current_pending_components[0]
            idx = chamber.index
            if wafer.type == WaferType.TYPE_A:
                factor = chamber_speed_factors_A[idx]
            if wafer.type == WaferType.TYPE_B:
                factor = chamber_speed_factors_B[idx]
            if wafer.type == WaferType.TYPE_C:
                factor = chamber_speed_factors_C[idx]
            if wafer.type == WaferType.TYPE_D:
                factor = chamber_speed_factors_D[idx]
            return int(wafer.process_time * factor)
    ...
    def _complete_vtr_action(self, component):
        ...
        if vtr_action == VTRAction.ENTRY_TO_LEFT_ARM:
            ...
            chambers[chamber_index].process_time = self.getCMBExeTime(chambers[chamber_index].holding_wafer)
            ...
            # force purge: 웨이퍼를 처리한 후 항상 청소 수행
